...
\begin{proof}
	...
	Conversely, if \eqref{SOS:sdp:eq} is fulfilled, then we have $f(X)= m(X)^\top Z m(X)$. Since $Z$ is psd, we can write it as $Z = \sum_{j=1}^r u_j u_j^\top$ for some finitely many vectors $u_1,\ldots,u_r \in \R^{E_d^n}$. This yields \blue{$f=f_1^2 + \cdots + f_j^2$} for $f_j = \sprod{m(X)}{u_j}$. 
\end{proof}
...
The total number of variables of the latter system is \blue{$k^2+1$}, where $k = \binom{n+d}{d}$ and the number of equations is $|E_{2d}^n| = \binom{n+2d}{2d}$. 
...
For \eqref{unconst:POP:dual:SOS:as:sdp:eq} to be feasible, we need to have $f - y>0$ on $\R^n$ for at least one $y \in \R$. With such choice $\newt(f-y) = \conv(\newt(f) \cup \{0\})$. So, by \blue{Proposition~\ref{newton:SOS}},  for feasibility it is necessary that $\conv(\newt(f) \cup \{0\})$ is of the form $2 P$, where $P$ is a polytope with integer vertices (so called integer polytope). \blue{Proposition~\ref{newton:SOS}} also shows that in that case, we can use $m(X)$ of the form $m(X) = (X^\alpha)_{\alpha \in P \cap \Z^n}$. So, rather than using \blue{exponent vectors} from the whole set $E_n^d$, we can restrict ourselves to $P \cap \Z^n$ (a subset of $E_n^d$), so that the size of the SDP \eqref{unconst:POP:dual:SOS:as:sdp:eq} gets reduced. 
...
\begin{proof}	
	...
	\[
		\frac{1 + x_1^2 x_2^4 + x_1^4 x_2^4}{3} \ge \sqrt[3]{ 1 (x_1^2 x_2^4) (x_1^4 x_2^2)} = x_1^2 x_2^2
	\]
	...
	(d): The proof of (c) can be used without any changes if $y \ne 1$. If $y=1$, then the Newton polytope of $f$ gets different, because the constant term disappears. In this case, however we see that $f -y $ is not SOS, because $f$ attains negative values. We get $f-1 = X_1^2 X_2^2 (-3 + \blue{X_1^2} + X_2^2)$. So $f(x_1,x_2)<0$ if $x_1,x_2 \in \R \setminus \{0\}$ and the distance of $(x_1,x_2)$ to $(0,0)$ is strictly less than $\sqrt{3}$. 
\end{proof}
...
\end{lemma}
\begin{proof}
	Changing coordinates and rescaling, we can assume that $q = X^2 + 1$. Since $f$ is non-negative \blue{and non-zero}, its degree is at least two. 
