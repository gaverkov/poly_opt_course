...
\begin{solution} 
	...
	Look at the vector exponents and the Newton polytope: 
		\begin{center}
			\begin{tikzpicture}[scale=0.5]
			\draw[black!30!white,line width=1pt] (0,0) grid (2,4);
			\draw[line width=1.2pt] (0,0) -- (2,0) -- (2,4) -- cycle;
...
\begin{proof}	
	...
	\[
		f = \sum_{j=1}^r f_j^2 = \sum_{j=1}^r \sum_{\alpha, \beta \in E} c_{j,\alpha} X^{\alpha + \beta}. 
	\]
	Thus, if we are interested in expressing a coefficient at the monomial $X^\gamma$ of the polynomial through the coefficients of $f_j$'s, we need to check for representations $\gamma = \alpha + \beta$ with $\alpha, \beta \in E$. It's nice to view the latter equation as an equation for the midpoints of segments with vertices in $2 E$, by rewriting it as $\gamma = \frac{1}{2}(2 \alpha + 2 \beta)$, where $2 \alpha, 2 \beta \in E$. 
	\begin{center}
		\begin{tikzpicture}[scale=0.5]
		\draw[black!20!white] (0,0) grid (4,4);
	...
	(d): The proof of (c) can be used without any changes if $y \ne 1$. If $y=1$, then the Newton polytope of $f$ gets different, because the constant term disappears. In this case, however we see that $f -y $ is not SOS, because $f$ attains negative values. We get $f-1 = X_1^2 X_2^2 (-3 + X_2^2 + X_2^2)$. So $f(x_1,x_2)<0$ if $x_1,x_2 \in \R \setminus \{0\}$ and the distance of $(x_1,x_2)$ to $(0,0)$ is strictly less than $\sqrt{3}$. 
\end{proof}
